...
Type.forSchema = function (schema, opts) {
  ...
  if (schema.logicalType && opts.logicalTypes && !LOGICAL_TYPE) {
    ...
    if (DerivedType) {
      ...
      try {
        console.debug('instantiating logical type for ' + schema.logicalType);
        return new DerivedType(schema, opts);
      } catch (err) {
        console.debug('failed to instantiate logical type for ' + schema.logicalType);
...
function EnumType(schema, opts) {
  ...
  if (this.default !== undefined && this._indices[this.default] === undefined) {
    throw new Error('invalid ' + this + ' default: ' + JSON.stringify(this.default));
  }
  this._branchConstructor = this._createBranchConstructor();
  Object.freeze(this);
}
...
RecordType.prototype._update = function (resolver, type, opts) {
  ...
  for (i = 0; i < rFields.length; i++) {
    ...
    if (matches.length > 1) {
      throw new Error('ambiguous aliasing for ' + type.name + '.' + field.name + ' (' + matches + ')');
    }
    if (!matches.length) {
      if (field.defaultValue() === undefined) {
...
function Field(schema, opts) {
  ...
  if (value !== undefined) {
    ...
    try {
      ...
      var msg = 'incompatible field default ' + JSON.stringify(value) + ' (' + err.message + ')';
      if (Type.isType(type, 'union')) {
        msg += ', union defaults must match the first branch\'s type (' + JSON.stringify(type.types[0]) + ')';
      }
      throw new Error(msg);
    }
...
function throwInvalidError(val, type) {
  throw new Error('invalid ' + JSON.stringify(type.schema()) + ': ' + JSON.stringify(val));
}
